    ...
    public class CorridorNode : Node
    {
        ...
        private void ProcessRoomOnHorizontal(Node node1, Node node2)
        {
            ...
            int y = GetValidYForNeighbourHorizontal(leftNode.TopLeftAreaCorner, leftNode.BottomRightAreaCorner,
                rightNode.TopLeftAreaCorner,
                rightNode.BottomLeftAreaCorner);

            while (y == -1 && sortedLeftNodes.Count > 1)
            {
                sortedLeftNodes = sortedLeftNodes.Where(
                    child => child.TopLeftAreaCorner.y != leftNode.TopLeftAreaCorner.y).ToList();
                leftNode = sortedLeftNodes[0];

                y = GetValidYForNeighbourHorizontal(leftNode.TopLeftAreaCorner, leftNode.BottomRightAreaCorner, rightNode.TopLeftAreaCorner, rightNode.BottomLeftAreaCorner);
            }
        ...
        private void ProcessRoomOnVertical(Node node1, Node node2)
        {
            ...
            {
                bottomNode = bottomNodeChildren[0];
            }
            else
            {
            ...
            List<Node> possibleNeighboursInTopNode = topNodeChildren.Where(
            ...
            if (possibleNeighboursInTopNode.Count == 0)
                topNode = node2;
            else
                topNode = possibleNeighboursInTopNode[0];

            int x = GetValidXForNeighbourVertical
            (
                bottomNode.TopLeftAreaCorner,
                bottomNode.TopRightAreaCorner,
                topNode.BottomLeftAreaCorner,
                topNode.BottomRightAreaCorner
            );
            ...
            while (x == -1 && sortedBottomNodes.Count > 1)
            {
                ...
                x = GetValidXForNeighbourVertical(bottomNode.TopLeftAreaCorner, bottomNode.TopRightAreaCorner, topNode.BottomLeftAreaCorner, topNode.BottomRightAreaCorner);
            }
        ...
        private int GetValidXForNeighbourVertical(Vector2Int bottomNodeLeft,
        ...
        {
            Vector2 middlePoint1 = ((Vector2)this._node1.TopRightAreaCorner + this._node1.BottomLeftAreaCorner) / 2;
            Vector2 middlePoint2 = ((Vector2)this._node2.TopRightAreaCorner + this._node2.BottomLeftAreaCorner) / 2;
            
            float relativeAngle = CalculateAngle(middlePoint1, middlePoint2);

            if ((relativeAngle < 45 && relativeAngle >= 0) || (relativeAngle > -45 && relativeAngle < 0))
                return RelativePositionType.Right;
            else if (relativeAngle > 45 && relativeAngle < 135)
                return RelativePositionType.Up;
            else if (relativeAngle > -135 && relativeAngle < -45)
                return RelativePositionType.Down;
            else
